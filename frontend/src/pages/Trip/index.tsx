...
export default function Trip() {
  ...
  const ddoonRef = useRef<HTMLImageElement | null>(null);
  ...
  useEffect(() => {
    ...
    if (challengeData) {
      ...
      img.onload = () => {
        if (containerRef.current) {
          ...
          if (challengeData.successCount === 0) {
            setTimeout(() => {
              if (containerRef.current) {
    ...
    if (ddoonRef.current) {
      setTimeout(() => {
        ddoonRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      }, 100);
    }
  }, [challengeData]);
          ...
          >
            {Array.from({ length: challengeData.days }, (_, index) => (
              ...
              >
                {index === challengeData.successCount - 1 && (
                  <>
                    <img
                      ...
                      src={Images.기본뚠뚠}
                      alt="기본뚠뚠"
                      className={`absolute ${isMobile ? 'w-[40%]' : 'w-[50%]'} bottom-[75%] object-contain z-20`}
                    />
                    {challengeData.successCount < challengeData.days &&
                      challengeData.lastSuccessDate.split('T')[0] === getTodayDate() && (
                        <p
                          className={`absolute ${isMobile ? 'bottom-[160%]' : 'bottom-[210%]'}  right-[-30%] text-center z-20`}
                        >
                          내일도 화이팅!
                        </p>
                      )}
                ...
                <img src={Images.cloud} className={`${isMobile && 'w-[85%]'}`} alt="구름" />
